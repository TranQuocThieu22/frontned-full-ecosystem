...
import F_rihlieosdm_Create from "./F_rihlieosdm_Create";
import F_rihlieosdm_Delete from "./F_rihlieosdm_Delete";
import F_rihlieosdm_Update from "./F_rihlieosdm_Update";

export interface I_rihlieosdm {
    promulgateDate: string | Date; // Ngày ban hành quyết định
    id: number; // STT
    decisionNumber: string; // Số quyết định
    decisionName: string; // Tên quyết định
    signer: string; // Người ký
    fileLink: string; // Link to file
}
...
export default function F_rihlieosdm_Read() {
    // Sử dụng useQuery để lấy dữ liệu
    const query = useQuery<I_rihlieosdm[]>({
    ...
    const columns = useMemo<MRT_ColumnDef<I_rihlieosdm>[]>(() => [
        {
