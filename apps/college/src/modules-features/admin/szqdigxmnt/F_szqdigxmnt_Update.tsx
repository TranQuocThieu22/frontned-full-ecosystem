...
interface ITable_szqdigxmnt_Create {
    id?: number //STT
...
export default function F_szqdigxmnt_Update({ data }: { data: F_szqdigxmnt_Update }) {
    const form = useForm<F_szqdigxmnt_Update>({
        initialValues: data
    })

    // Sử dụng useQuery để lấy dữ liệu
    const query = useQuery<ITable_szqdigxmnt_Create[]>({
    ...
    const columns = useMemo<MRT_ColumnDef<ITable_szqdigxmnt_Create>[]>(
        () => [
            {
                header: "Mã sinh viên",
    ...
    return (
        <MyActionIconUpdate form={form} onSubmit={() => { }} modalSize={"80%"}>
            <MyFlexRow>
                <MyTextInput disabled label="Số quyết định" {...form.getInputProps("soQuyetDinh")} />
                <MySelect data={['Quyết định trúng tuyển', 'Quyết định thăng chức']} defaultValue={'Quyết định trúng tuyển'} label="Lý do quyết định" {...form.getInputProps("lyDoQuyetDinh")} />
            </MyFlexRow>
                <MyTextInput label="Tên" {...form.getInputProps("tenQuyetDinh")} />
            <MyFlexRow>
                <MyDateInput label="Ngày quyết định" {...form.getInputProps("ngayQuyetDinh")} />
                <MySelect data={['Hiệu trưởng - Nguyễn Hữu Quốc', 'Hiệu phó - Trương Văn B']} defaultValue={'Hiệu trưởng - Nguyễn Hữu Quốc'} label="Người ký" />
            </MyFlexRow>
