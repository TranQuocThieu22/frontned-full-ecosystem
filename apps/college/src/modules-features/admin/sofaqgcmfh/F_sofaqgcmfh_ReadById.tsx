...
import F_sofaqgcmfh_Setting from "./F_sofaqgcmfh_Setting";

interface IsofaqgcmfhIdViewModel {
    id?: number;
    hkThu?: number;
    maMonHoc?: string;
    name?:string;
...
const mockData: IsofaqgcmfhIdViewModel[] = [
...
const mockData2: IsofaqgcmfhIdViewModel[] = [
    {
        id: 5,
...
export default function F_sofaqgcmfh_ReadById(
    { selectedRowIdList }: { selectedRowIdList: number[] }
) {
    let [listSelectedId, setListSelectedId] = useState<number[]>([]);
    const AllsofaqgcmfhId = useQuery<IsofaqgcmfhIdViewModel[]>({
        queryKey: [`F_sofaqgcmfh_ReadById`, listSelectedId],
        queryFn: async () => {
    ...
    const columns = useMemo<MRT_ColumnDef<IsofaqgcmfhIdViewModel>[]>(() => [
        {
            header: "HK Thứ",
            accessorKey: "hkThu",
        },
        ...
        {
            header: "Điều kiện TC",
            accessorFn: (row) => <F_sofaqgcmfh_Setting />
        },
    ...
    if (AllsofaqgcmfhId.isLoading) return "Đang tải dữ liệu...";
    if (AllsofaqgcmfhId.isError) return "Không có dữ liệu...";
