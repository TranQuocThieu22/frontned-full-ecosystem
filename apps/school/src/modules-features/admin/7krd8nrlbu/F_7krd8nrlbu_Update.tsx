...
import { MyActionIconUpdate, MyButton, MyButtonCreate, MyCenterFull, MyCheckbox, MyDataTable, MyDateInput, MyFieldset, MySelect, MyTextArea, MyTextInput } from "aq-fe-framework/components"
...
export interface I_7krd8nrlbu_Update {
    id?: number;
...
export default function F_hbwyvjoqgu_Update({ data }: { data: I_7krd8nrlbu_Update }) {
      
    const dis = useDisclosure(); 
    const form = useForm<I_7krd8nrlbu_Update>({
        initialValues: {
            ...data,
            gioKhoiHanh: data.gioKhoiHanh instanceof Date ? formatTime(data.gioKhoiHanh) : data.gioKhoiHanh ?? "",
            gioDenDuKien: data.gioDenDuKien instanceof Date ? formatTime(data.gioDenDuKien) : data.gioDenDuKien ?? "",
        },
        validate: {
    ...
    return (
        <MyActionIconModal modalSize={1000} crudType={"update"} title='Chi tiết lịch trình xe' onSubmit={() => { } } disclosure={dis}  >
            <form onSubmit={form.onSubmit((values) => console.log(values)) }>
                <MyDateInput withAsterisk label='Ngày tạo lịch trình' {...form.getInputProps("ngayTaoLichTrinh")}/>
                <MyTextInput disabled withAsterisk  label='Mã tuyến xe '  {...form.getInputProps("maTuyenXe")} />
                <MyTextInput disabled withAsterisk  label='Tên tuyến xe '  {...form.getInputProps("tenTuyenXe")} />
                <MySelect withAsterisk data={['Đón khách', 'Trả khách']}
