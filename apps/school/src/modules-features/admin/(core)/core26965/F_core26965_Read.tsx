...
import F_core26965_Delete from "./F_core26965_Delete";
import F_core26965_Update from "./F_core26965_Update";
interface I {
    documentType?: number;
    id?: number;
    code?: string;
...
export default function F_core26965_Read() {
    const documentAttributeQuery = useQuery<I[]>({
        queryKey: ['F_core26965_Read', OBJECT_DOCUMENT_TYPES.Regulations],
        queryFn: async () => {
            const result = await baseAxios.get("/DocumentAttribute/GetByType?", { params: { documentType: OBJECT_DOCUMENT_TYPES.Regulations } })
            ...
            <Accordion.Panel>
                <MyDataTable
                    ...
                    renderRowActions={({ row }) => (
                        <MyCenterFull>
                            <F_core26965_Update values={row.original!} />
                            <F_core26965_Delete id={row.original.id!} />
                        </MyCenterFull>
                    )}
                />
            </Accordion.Panel>
