...
export default function F_core27311_Read() {
    const documentAttributeQuery = useQuery<I[]>({
        queryKey: ['F_core27311_Read', OBJECT_DOCUMENT_TYPES.Workflow],
        queryFn: async () => {
            const result = await baseAxios.get("/DocumentAttribute/GetByType?", { params: { documentType: OBJECT_DOCUMENT_TYPES.Workflow } })
            ...
            <Accordion.Panel>
                <MyDataTable
                    ...
                    renderRowActions={({ row }) => (
                        <MyCenterFull>
                            <F_core27311_Update values={row.original!} />
                            <F_core27311_Delete id={row.original.id!} />
                        </MyCenterFull>
                    )}
                />
            </Accordion.Panel>
