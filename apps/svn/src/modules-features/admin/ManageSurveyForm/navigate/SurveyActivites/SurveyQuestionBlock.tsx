...
interface SurveyQuestionBlockProps {
  ...
  onDelete: () => void;
  conditionType?: "Và" | "Hoặc";
  onConditionChange?: (val: "Và" | "Hoặc") => void;
  showCondition?: boolean; // chỉ show ở câu 2 trở đi
}
...
export default function SurveyQuestionBlock({
  ...
  function showRequired (condition: boolean){
    ...
    <Select
            ...
            value={conditionType}
            onChange={(v) => {
              if (v === "Và" || v === "Hoặc") onConditionChange?.(v);
            }}
            size="xs"
            radius="md"
            fw={600}
