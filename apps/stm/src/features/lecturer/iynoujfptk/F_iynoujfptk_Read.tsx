...
import { dashboardLecturerService } from "@/shared/APIs/dashboardLecturerService";
...
export default function F_iynoujfptk_Read() {
    ...
    const queryDashboardLecturer = useCustomReactQuery({
        queryKey: ["dashboardLecturer", lecturerId],
        axiosFn: () => dashboardLecturerService.getLecturer({ lecturerId }),
    });
                ...
                <Flex direction="column" gap={"sm"} h={{ base: 'auto', sm: '100%' }} mih={{ base: 'auto', sm: '100vh' }}>
                    ...
                    <Flex display={{ base: 'none', sm: 'none', md: 'none', lg: 'block' }}>
                        <F_iynoujfptk_HeaderInfo
                            data={{
                                totalOngoingCourseSection: queryDashboardLecturer.data?.totalOngoingCourseSection,
                                totalRequiringAttendance: queryDashboardLecturer.data?.totalRequiringAttendance,
                                totalTeachingSessions: queryDashboardLecturer.data?.totalTeachingSessions,
                            }}
                        />
                    </Flex>
                        ...
                        <Grid.Col span={{ base: 12, sm: 12, md: 5, lg: 5 }}>
                            <Card
                                ...
                                <F_iynoujfptk_CourseList
                                    courses={queryDashboardLecturer.data?.courseSections?.map((course) => ({
                                        id: course.id,
                                        name: course.name,
                        ...
                        <Grid.Col span={{ base: 12, sm: 12, md: 7, lg: 7 }}>
                            <Card
                                ...
                                <F_iynoujfptk_PieChart
                                    totalCompletedCourseSection={queryDashboardLecturer.data?.totalCompletedCourseSection}
                                    totalOngoingCourseSection={queryDashboardLecturer.data?.totalOngoingCourseSection}
                                    totalUpcomingCourseSection={queryDashboardLecturer.data?.totalUpcomingCourseSection}
                                />
                            </Card>
                        </Grid.Col>
                    </Grid>
