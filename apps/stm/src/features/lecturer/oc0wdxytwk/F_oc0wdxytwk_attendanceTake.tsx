...
import { CourseSectionSchedule } from "@/shared/interfaces/CourseSectionSchedule";
import { StudentAttendence } from "@/shared/interfaces/studentAttendence";
...
export default function F_oc0wdxytwk_attendanceTake({
  ...
  courseData: CourseSectionSchedule;
  totalDate: string;
}) {
  const dis = useDisclosure();
  const QueryClient = useQueryClient();
  const [Attendance, setAttendance] = useState<StudentAttendence[]>([]);
  const [lecturerReview, setLecturerReview] = useState<string | null>(null);
  const studentData = useQuery<StudentAttendence[]>({
    ...
    queryFn: async () => {
      ...
      if (response.data.data) {
        let updatedData = response.data.data.map((item: StudentAttendence) => {
  ...
  const columns = useMemo<MRT_ColumnDef<StudentAttendence>[]>(
    () => [
      {
        header: "Họ tên",
  ...
  const handleFieldChange = (row: StudentAttendence, fieldName: keyof StudentAttendence, fieldValue: any) => {
