...
import { CourseRegistrationViewModel, ExamRegistrationViewModel, RegisterAndPaymentViewModel } from './Interfaces/Interfaces';
...
export default function SelectPaymentMethodSection({ navigateFunction }: { navigateFunction: (step: number) => void }) {
  ...
  const submitRegistrationService = async (newUserId?: number) => {
    let courseRegistration: CourseRegistrationViewModel[] = []
    let examRegistration: ExamRegistrationViewModel[] = []
    ...
    let receiptPDFFileB64 = null

    let inputData: RegisterAndPaymentViewModel = {
      id: 0,
      code: null,
      name: null,
      concurrencyStamp: "string",
