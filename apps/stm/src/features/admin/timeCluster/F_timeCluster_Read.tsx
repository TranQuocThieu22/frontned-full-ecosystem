...
export default function F_timeCluster_Read() {
    const timeClusterQuery = useMyReactQuery({
        ...
        axiosFn: async () => timeClusterService.getAll({
    ...
    const columns = useMemo<MRT_ColumnDef<TimeCluster>[]>(() => [
        {
        ...
        },
        {
            header: "Tiết bắt đầu",
            accessorFn: (row) => {
                const tietBatDaus = row.timeClusterDetails?.map((item:any) => item.classPeriodStart)
                return tietBatDaus
            },
            Cell: ({ cell }) => {
                return (
