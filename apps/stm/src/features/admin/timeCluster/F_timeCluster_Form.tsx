...
export default function F_timeCluster_Form({ values }: { values?: IValues }) {
    ...
    const timeTypeDetailQuery = useMyReactQuery({
        ...
        axiosFn: async () => timeTypeService.getAll({
            params: "?cols=TimeTypeDetails"
        })
    })
    const createMutation = useMyReactMutation({
        axiosFn: (values: Partial<TimeCluster>) => timeClusterService.create(values)
    })
    const updateMutation = useMyReactMutation({
        axiosFn: (values: Partial<TimeCluster>) => timeClusterService.update(values)
    })
        ...
        form.setValues({
            ...
            danhSachBuoiHoc: values.timeClusterDetails?.map(item => {
                ...
                return ({
                    ...
                    soPhut: timeTypeDetailQuery.data[0]?.timeTypeDetails?.slice(item.classPeriodStart, item.classPeriodEnd! + 1).reduce((acc: any, cur: any) => acc + cur.minuteNumber!, 0),
    ...
    const transformToTimeClusterDetails = (values: IBuoiHoc[]): TimeClusterDetails[] => {
