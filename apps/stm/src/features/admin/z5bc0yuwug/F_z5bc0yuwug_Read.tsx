...
import { priceConfigService, type PriceConfig } from "@/shared/APIs/priceConfigService";
...
export default function F_z5bc0yuwug_Read() {
    ...
    const priceConfigQuery = useCustomReactQuery({
        queryKey: ["priceConfigs", loaiThu?.value],
        axiosFn: () =>
            priceConfigService.getByType({
                type: parseInt(loaiThu?.value!),
            }),
    })

    const columns = useMemo<MRT_ColumnDef<PriceConfig>[]>(() => [
        {
    ...
    return (
        <MyFlexColumn>
            ...
            />
            <CustomFieldset title={"Danh sách công thức thu"}>
                <MyDataTable
                    columns={columns}
