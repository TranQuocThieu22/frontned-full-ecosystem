...
import { UserBranch } from "@/shared/interfaces/userBranch";
import { UserProgram } from "@/shared/interfaces/userProgram";
import { UserSkillCenter } from "@/shared/interfaces/userSkillCenter";
...
export default function F_jdtonxkhjl_Update({ values }: { values: Lecturer }) {
    const disclosure = useDisclosure();
    const QueryClient = useQueryClient()
    const [selectedSkillCenter, setselectedSkillCenter] = useState<UserSkillCenter>(
        values.userSkillCenters?.[0] ?? ({} as UserSkillCenter)
    );
    const [unselectedSkillCenter, setUnselectedSkillCenter] = useState<UserSkillCenter>(
    );
    const [selectedPrograms, setselectedPrograms] = useState<UserProgram[] | null>(values.userPrograms ?? []);
    const [unselectedPrograms, setUnselectedPrograms] = useState<UserBranch[] | null>([]);

    const [selectedBranches, setselectedBranches] = useState<UserBranch[] | null>(values.userBranch ?? []);
    const [unselectedBranches, setUnselectedBranches] = useState<UserBranch[] | null>([]);
    ...
    useEffect(() => {
        ...
        setselectedSkillCenter(values.userSkillCenters?.[0] ?? ({} as UserSkillCenter));
                                ...
                                const SkillcenterData = skillCenterQuery.data
                                    ...
                                    .map((item) => {
                                        ...
                                        return {
                                            ...
                                            code: item.code ?? "",
                                            name: item.name ?? "",
                                            concurrencyStamp: existingSkillCenter?.concurrencyStamp ?? "",
                                            isEnabled: true,
                                        };
                                    })[0] || ({} as UserSkillCenter);
                                ...
                                programQuery.data
                                    ...
                                    .map((item) => {
                                        ...
                                        return {
                                            ...
                                            code: item.code ?? "",
                                            name: item.name ?? "",
                                            concurrencyStamp: existingProgram?.concurrencyStamp ?? "",
                                            isEnabled: true,
                                        };
                                    }) || [];
                                ...
                                branchQuery.data
                                    ...
                                    .map((item) => {
                                        ...
                                        return {
                                            ...
                                            code: item.code ?? "",
                                            name: item.name ?? "",
                                            concurrencyStamp: existingBranch?.concurrencyStamp ?? "",
                                            isEnabled: true,
                                        };
                                    }) || [];
