...
import { Button, Center, Checkbox, Group, NumberFormatter, NumberInput, Select, SimpleGrid, Text } from "@mantine/core";
...
import AssignStudentToCourseSectionStudentsByCourse from "./AssignStudentToCourseSectionStudentsByCourse";
import AssignStudentToCourseSectionDeleteSection from "./AssignStudentToCourseSectionDeleteSection";
import AssignStudentToCourseSectionStudentsBySection from "./AssignStudentToCourseSectionStudentsBySection";
...
export default function AssignStudentToCourseSectionTable() {
    ...
    const DsLopQuery = useQuery<IDsLop[]>({
        queryKey: [`AssignStudentToCourseSectionDsLop`],
        queryFn: async () => {
    ...
    const [selectedRows, setSelectedRows] = useState<number[]>([17]);

    const filterCoursesBySelection = (cumThoiGianHoc: any, selectedRows: number[]): number[] => {
        if (!cumThoiGianHoc) return [];
    ...
    const transformedData = cumThoiGianHoc.data?.flatMap(course =>
        course.courseTimeClusters.map(cluster => ({
            ...course,
            id: cluster.id,
            timeClusterName: cluster.timeCluster.name,
            soLuongHocVienDangKy: cluster.courseSectionNumberTotal,
            soLuongDaXepLop: cluster.courseSectionNumber,
            daXepLop: cluster.courseSectionNumberTotal > 0,
        }))
    );

    const columnsCumThoiGianHoc = useMemo<CustomColumnDef<CourseTimeClusterData>[]>(() => [
        {
            header: "Mã khóa học",
            accessorKey: "code"
        },
        ...
        },
        {
            ...
            accessorKey: "price",
            accessorFn(originalRow) {
                return <NumberFormatter value={originalRow.price!} thousandSeparator />;
            }
        },
        {
            header: "Cụm thời gian",
            accessorKey: "timeClusterName"
        },
        {
        ...
        },
        {
            ...
            accessorFn(originalRow) {
                return (
                    <Group>
                        <AssignStudentToCourseSectionStudentsByCourse label="Xem" color="indigo" values={originalRow} />
                    </Group>
                );
            }
        },
    ...
    const columnsDsLop = useMemo<CustomColumnDef<any>[]>(
        () => [
            {
                header: "Mã lớp",
                accessorKey: "code"
            ...
            },
            {
                ...
                accessorFn(originalRow) {
                    return (
                        <Group>
                            <AssignStudentToCourseSectionStudentsBySection label="Xem/Hiệu chỉnh" color="yellow" values={originalRow} />
                        </Group>
                    );
                }
            },
    ...
    const form = useForm<IChiaLop>({
        ...
        validate: {
            siSoToiDaTrenPhongHoc: (value) => {
                if (value <= 0) return 'Sĩ số phải lớn hơn 0';
                return null;
            }
        }
    })
    ...
    return (
        <>
                        ...
                        <Button
                            ...
                            onClick={async () => {
                                ...
                                const response = await baseAxios.post("/courseSection/CreateMutipleCourseSection", {
                                    ...
                                    "courseTimeClusterIds": filteredCourses
                                });
            ...
            {cumThoiGianHoc.isSuccess &&
                <CustomFieldset mt="10" title="Danh sách khóa học theo cụm thời gian học">
                    <CustomDataTable
                        initialState={{
            ...
            {DsLopQuery.data &&
                <CustomFieldset mt="10" title="Danh sách lớp">
                    <CustomDataTable
                        enableRowSelection={true}
                        ...
                        renderRowActions={({ row }) => (
                            <Center>
                                <AssignStudentToCourseSectionDeleteSection data={row.original} />
                            </Center>
                        )}
                    />
                </CustomFieldset>
