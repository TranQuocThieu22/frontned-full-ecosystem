...
import F10_2DeleteStudentFromExam from './F10_2DeleteStudentFromExam';
...
export default function F10_2StudentListByExam({ label, values, color }: IBase) {
    const disc = useDisclosure()
    const studentListQuery = useQuery<IEnrollment[]>({
        queryKey: [`F10_2StudentListByExam`, values],
        queryFn: async () => {
    ...
    const columns = useMemo<MRT_ColumnDef<IEnrollment>[]>(
        () => [
            ...
            },
            {
                ...
                accessorFn(originalRow) {
                    return (<Group>
                        <Anchor>{originalRow.courseSection == null || (originalRow.courseSection as ICourseSection).course == null ? "" : (originalRow.courseSection as ICourseSection).course.code}</Anchor>
                    </Group>);
                } // M
            },
            ...
            },
            {
                ...
                accessorFn: (originalRow) => {
                    return (
                        <Text  >{originalRow.courseSection?.course?.courseTimeClusters == null ? "" : originalRow.courseSection?.course.courseTimeClusters
                    ...
                    renderRowActions={({ row }) => {
                        ...
                        return (
                            <MyCenterFull>
                                ...
                                <F10_2DeleteStudentFromExam data={row.original} />
