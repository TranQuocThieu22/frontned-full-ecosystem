...
import TeachingAssignmentDeleteBtn from './TeachingAssignmentDeleteBtn';
...
export default function StudentListByClass({ label, values, color }: IBase) {
    const disc = useDisclosure()

    const danhSachLop = useQuery<IEnrollment[]>({
        queryKey: [`F8_2StudentListByClass`, values],
        queryFn: async () => {
    ...
    const columns = useMemo<MRT_ColumnDef<IEnrollment>[]>(
        () => [
            ...
            },
            {
                ...
                accessorFn(originalRow) {
                    return (<Group>
                        <Anchor>{originalRow.courseSection == null ? "" : originalRow.courseSection.course.code}</Anchor>
                    </Group>);
                } // M
            },
...

            {
                ...
                accessorFn(originalRow) {
                    return (<Group>
                        <Anchor>{originalRow.courseSection == null ? "" : originalRow.courseSection.course.name}</Anchor>
                    </Group>);
                } // M
            },
            {
                    ...
                    renderRowActions={({ row }) => {
                        ...
                        return (
                            <MyCenterFull>
                                ...
                                <TeachingAssignmentDeleteBtn data={row.original} />
