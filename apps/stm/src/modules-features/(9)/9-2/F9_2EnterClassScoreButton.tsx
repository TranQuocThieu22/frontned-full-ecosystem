...
import { AQButtonCreateByImportFile, MyButtonModal, MyDataTable, MyFlexColumn, MyNumberInput } from 'aq-fe-framework/components';
import { utils_date_dateToDDMMYYYString, utils_notification_show } from 'aq-fe-framework/utils';
import { MRT_ColumnDef } from 'mantine-react-table';
import { useEffect, useMemo, useState } from 'react';
import F9_2ClassInfo from './F9_2ClassInfo';


export default function F9_2EnterClassScoreButton(
    ...
    const columns = useMemo<MRT_ColumnDef<ITest>[]>(() => [
            ...
            })) ?? []),
        {
            ...
            //     // Ensure scoreConfigs is available
            //     const scoreConfigs = programDetailQuery.data?.scoreConfigs
            //         ?.filter(config => config.scoreType === 1) as IScoreConfig[] | undefined;
            //     if (!scoreConfigs) return <Text>Không có dữ liệu</Text>;
            ...
            //         const score = row[config.code];
            //         if (typeof score === 'number') {
            //             const weight = config.percentScore / 100;
            ...
            //     return <Text>{`${totalWeightedScore.toFixed(2)} - ${finalScore.toFixed(2)}`}</Text>;
            // }
        },
        {
            header: "Đạt",
        ...
        <>
            <MyButtonModal
                ...
                <form onSubmit={form.onSubmit((values) => {
                    ...
                    <MyFlexColumn >
                        {programDetailQuery.data && (
                            <F9_2ClassInfo data={data} programDetailData={programDetailQuery.data} />
                        )}
