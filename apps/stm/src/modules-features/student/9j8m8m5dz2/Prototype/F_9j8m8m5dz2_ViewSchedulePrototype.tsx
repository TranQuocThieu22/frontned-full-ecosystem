...
export default function F_9j8m8m5dz2_ViewSchedule() {
  ...
  const query = useQuery<I[]>({
    queryKey: [`F_9j8m8m5dz2_ViewSchedule`],
  ...
  useEffect(() => {
    // get all events
    if (query.data == undefined || !query.data || query.data.length === 0) return
    const result = query.data.flatMap(item => generatePeriod(item))
    if (result.length > 0) {
    ...
    <div>
      <ScheduleXCalendar calendarApp={calendar} customComponents={{
        timeGridEvent: ({ calendarEvent }: { calendarEvent: I }) => {
          return (
            <Box h={'100%'} style={{ border: "1px solid gray", borderRadius: "5px", boxShadow: "rgba(0, 0, 0, 0.4) 0px 2px 4px, rgba(0, 0, 0, 0.3) 0px 7px 13px -3px, rgba(0, 0, 0, 0.2) 0px -3px 0px inset" }} bg={calendarEvent.laLichThi ? 'violet.2' : calendarEvent.color} p={4}>
              <MyFlexColumn gap={1}>
          ...
          return (
            <Paper p={'md'} style={{ boxShadow: "rgba(0, 0, 0, 0.4) 0px 2px 4px, rgba(0, 0, 0, 0.3) 0px 7px 13px -3px, rgba(0, 0, 0, 0.2) 0px -3px 0px inset" }}>
              <MyFlexColumn>
                ...
                <MyFlexRow>
                  <IconClock />
                  <Text>{utils_date_formatToDateTimeStartEnd(new Date(`${calendarEvent.startDate} ${calendarEvent.schedule?.timeStart}`), new Date(`${calendarEvent.endDate} ${calendarEvent.schedule?.timeEnd}`))}</Text>
                </MyFlexRow>
                <MyFlexRow>
                  <IconUsersGroup />
