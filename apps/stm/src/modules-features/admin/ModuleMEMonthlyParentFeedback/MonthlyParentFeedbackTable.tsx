...
export default function MonthlyParentFeedbackTable() {
    ...
    const columns = useMemo<MRT_ColumnDef<IClassManagement>[]>(() => [
        ...
        { header: "Sĩ số hiện tại/Sĩ số tối đa", accessorKey: "studentCount" },
        {
            ...
            accessorKey: "status",
            size: 250,
            accessorFn: row => <Center><AcitivityStatusBadge result={AcitivityStatusMap[row.status || -1]?? 0} /></Center>
        },
        {
        ...
        },
        {
            ...
            id: "qcApproval",
            size: 250,
            accessorFn: row => <Center><QLCLApprovalStatusBadge result={QLCLApprovalStatusMap[row.qcApproval || -1]?? 0} /></Center>
        }
    ], []);
