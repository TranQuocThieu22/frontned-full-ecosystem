...
import CourseCard, { CourseCardProps } from "@/modules-features/main/_components/CourseCard";
import CourseExamCard from "@/modules-features/main/_components/CourseExamCard";
import { SimpleGrid, Stack, Title } from "@mantine/core";
import { useQuery } from "@tanstack/react-query";

export default function Page(
) {
    const coursesData = useQuery<any[]>({
        queryKey: [`F3_3ReadCourseModule`],
        queryFn: async () => {
            return courses;
        },
    })
    if (coursesData.isLoading) return "Đang tải dữ liệu...";
    if (coursesData.isError) return "Không có dữ liệu...";
    return (
        <MyPageContent canBack title="Đăng ký khóa học">
            <Stack>
                <Title mt={12} mb={18} order={2}>Danh sách các khóa học có thể đăng ký</Title>
                ...
                <Title mt={48} mb={18} order={2}>Danh sách các khóa thi có thể đăng ký</Title>
                <SimpleGrid cols={{ base: 1, xs: 2, md: 3, lg: 4, xl: 5 }}>
                    {courses2.map((course, index) => {
                        return (
                            <CourseExamCard
                                key={index}
                                href={`/main/examRegistration/1`}
                                image={course.image}
                                title={course.title}
                                description={course.description}
                                fee={course.fee}
...
const courses2: CourseCardProps[] = [
    {
