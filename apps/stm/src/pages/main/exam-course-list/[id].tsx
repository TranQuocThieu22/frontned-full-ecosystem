...
import CourseExamCard, { CourseExamCardProps } from "../../../modules-features/main/_components/CourseExamCard";


export default function Page() {
    return (
        <MyPageContent canBack title="Danh sách các khóa học có thể đăng ký">
            <SimpleGrid cols={{ base: 1, xs: 2, md: 3, lg: 4, xl: 5 }}>
                {courses.map((course, index) => {
                    return (
                        <CourseExamCard key={index} href={`/main/examRegistration/1`} image={course.image} title={course.title} description={course.description} fee={course.fee} openingDate={course.openingDate} buttonLabel="Đăng ký" />
                    )
                })}
            </SimpleGrid>
        </MyPageContent>
