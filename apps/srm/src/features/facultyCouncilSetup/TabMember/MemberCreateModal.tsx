...
import { keyValueOf } from "../shared/FacultyCoucilFunctions";

interface Props {
    handleAddAcceptanceMember: Function;
    acceptanceMembersData?: IUseMapRefController<string, SRMAcceptanceMember>;
}

export default function MemberCreateModal({ handleAddAcceptanceMember, acceptanceMembersData }: Props) {
    const disc = useDisclosure();

    const handleConfirmSelect = (listUser: SRMLecturer[]) => {
        const { messageSuccess, messageError } = handleAddAcceptanceMember(listUser);
        if (messageSuccess) {
            disc[1].close();
            notifications.show({
                autoClose: 10000,
        ...
        >
            <Shared_LecturerTable
                enableRowSelection={(row) => {
                    if (acceptanceMembersData) {
                        return !acceptanceMembersData.get(keyValueOf(row.original.id));
                    }
                    return true;
                }}
                mantineSelectCheckboxProps={({ row }) => {
                    const isDisabled = acceptanceMembersData
                        ? !!acceptanceMembersData.get(keyValueOf(row.original.id))
                        : false;
