...
import { sanitizeAcceptanceCouncilPayload } from "./shared/FacultyCoucilFunctions";

export default function FacultyCouncilSetupCreateModal() {
    ...
    const mutation = useMutation({
        mutationFn: async () => {
            ...
            data.type = EnumAcceptanceCouncilType.Faculty;
            ...
            if (response.data.isSuccess === 0) {
                ...
                if (dataMessage.Code) {
                    tabGeneralInfoFormRef.current?.setErrors({
                        code: "Hội đồng nghiệm thu cấp Khoa đã tồn tại",
                    }),
                ...
                queryClient.invalidateQueries({ queryKey: ['FacultyCouncilSetupTable', academicYearStore.state.academicYear?.id] });
                createDisc[1].close();
                notifications.show({
                    color: "green",
    ...
    return (
        <>
            ...
            <Modal
                size={"80%"}
                title={"Tạo hội đồng nghiệm thu cấp Khoa"}
                opened={createDisc[0]}
                onClose={() => {
