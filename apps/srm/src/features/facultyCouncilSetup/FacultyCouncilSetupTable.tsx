...
import FacultyCouncilSetupCreateModal from "./FacultyCouncilSetupCreateModal";
import FacultyCouncilSetupDeleteButton from "./FacultyCouncilSetupDeleteButton";
import FacultyCouncilSetupDeleteListButton from "./FacultyCouncilSetupDeleteListButton";
import FacultyCouncilSetupImportButton from "./FacultyCouncilSetupImportButton";
import FacultyCouncilSetupUpdateModal from "./FacultyCouncilSetupUpdateModal";

export default function FacultyCouncilSetupTable() {
    ...
    const facultyCouncilSetupQuery = useCustomReactQuery({
        queryKey: ["FacultyCouncilSetupTable", academicYearStore.state.academicYear?.id],
        axiosFn: () => acceptanceCouncilService.GetAllByAcademicYearAndType({
            academicYearId: academicYearStore.state.academicYear?.id ?? -1,
            type: EnumAcceptanceCouncilType.Faculty // Cấp Khoa
        }),
        options: {
    ...
    const columns = useMemo<MRT_ColumnDef<SRMAcceptanceCouncil>[]>(() => [
        ...
        {
            header: "File quyết định thành lập hội đồng nghiệm thu cấp Khoa", accessorKey: "attachmentDetail", accessorFn: (row) => {
                return (
                    <CustomCenterFull>
    ...
    return (
        <>
            <CustomFieldset title={"Danh sách hội đồng nghiệm thu cấp Khoa"}>
                <CustomDataTable
                    enableRowSelection
                    enableRowNumbers={false}
                    isLoading={facultyCouncilSetupQuery.isLoading || facultyCouncilSetupQuery.isPending}
                    isError={facultyCouncilSetupQuery.isError}
                    columns={columns}
                    data={facultyCouncilSetupQuery.data || []}
                    renderTopToolbarCustomActions={({ table }) => {
                        ...
                        return (
                            <>
                                <FacultyCouncilSetupCreateModal />
                                <FacultyCouncilSetupImportButton />
                                <AQButtonExportData
                                    objectName="Danh sách nghiệm thu cấp Khoa"
                                    data={selectedRows.length > 0 ? selectedRows : facultyCouncilSetupQuery.data || []}
                                    exportConfig={exportConfig}
                                />
                                <FacultyCouncilSetupDeleteListButton
                                    values={selectedRows} table={table} isFetchingTable={facultyCouncilSetupQuery.isFetching} />
                            </>
                        )
                    }}
                    renderRowActions={({ row, table }) => {
                        return (
                            <CustomCenterFull>
                                <CustomActionIcon
                                    actionType="update"
                                    loading={facultyCouncilSetupQuery.isFetching}
                                    onClick={() => {
                                        setValueUpdate(row.original);
                                        updateDisc[1].open();
                                    }}
                                />
                                <FacultyCouncilSetupDeleteButton data={row.original} table={table} isLoading={facultyCouncilSetupQuery.isFetching} />
                            </CustomCenterFull>
                        );
                    }}
                />
            </CustomFieldset>
            <FacultyCouncilSetupUpdateModal values={valueUpdate} updateDisc={updateDisc} />
        </>
    )
}
