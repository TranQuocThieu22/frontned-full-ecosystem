...
export default function TabExternalMembers({
    internalMembersData,
    ...
    useEffect(() => {
        if (initValues?.srmDeclarationMemberExternals && initValues.srmDeclarationMemberExternals.length > 0) {
            // Clear existing menuData
            internalMembersData.clear();

            // Load menuData from initValues - chỉ load các thành viên enabled
            initValues.srmDeclarationMemberExternals.forEach((member) => {
