import { service_Account } from '@/shared/APIs/service_Account';
import { ITaskDetail } from '@/shared/interfaces/task/ITaskDetail';
import { useDisclosure } from '@mantine/hooks';
import { notifications } from '@mantine/notifications';
import { IconDotsVertical, IconSelect } from '@tabler/icons-react';
import { MRT_ColumnDef, MRT_Row } from 'mantine-react-table';
import { useCallback, useMemo } from 'react';
import { CustomButton } from '@aq-fe/core-ui/shared/components/button/CustomButton/CustomButton';
import { CustomButtonModal } from '@aq-fe/core-ui/shared/components/button/CustomButtonModal/CustomButtonModal';
import { CustomDataTable } from '@aq-fe/core-ui/shared/components/dataDisplay/CustomDataTable';
import { CustomCenterFull } from '@aq-fe/core-ui/shared/components/layout/CustomCenterFull';
import { CustomFieldset } from '@aq-fe/core-ui/shared/components/layout/CustomFieldset';
import { Account } from '@aq-fe/core-ui/shared/interfaces/Account';
import { useCustomReactQuery } from '@aq-fe/core-ui/shared/hooks/useCustomReactQuery';

interface Props {
    data: ITaskDetail;
    onSelect: (selectedReviewer: Account) => void;
}

export default function AssignedReviewer({ data, onSelect }: Props) {
    const disclosure = useDisclosure();

    const userQuery = useCustomReactQuery({
        queryKey: ['selfAssessmentReviewer_userList'],
        axiosFn: () => service_Account.getAllModule(),
        options: {
            enabled: disclosure[0],
            staleTime: 5 * 60 * 1000,
        }
    });

    const dataReviewer = userQuery.data;

    const columns = useMemo<MRT_ColumnDef<Account>[]>(
        () => [
            { header: "Mã tài khoản", accessorKey: "userName", size: 150 },
            { header: "Họ tên", accessorKey: "fullName" },
            { header: "Đơn vị", accessorKey: "workingUnit.name", size: 300 },
        ], []);

    const handleSelect = useCallback((row: MRT_Row<Account>) => {
        onSelect(row.original);
        notifications.clean();
        notifications.show({
            title: 'Đã chọn thành viên',
            message: `Đã chọn ${row.original.fullName ?? 'nhân sự'} cho tiêu chí ${data?.eaqCriteria?.code ?? 'này'}. Nhấn "Lưu" để xác nhận.`,
            color: 'green',
            autoClose: 3000,
        });

        disclosure[1].close();
    }, [data?.eaqCriteria?.code, disclosure, onSelect]);

    // Memoize processed menuData
    const processedData = useMemo(() => {
        return (dataReviewer || []).map((person: Account) => ({
            ...person,
        }));
    }, [dataReviewer]);

    return (
        <CustomButtonModal
            disclosure={disclosure}
            isActionIcon
            actionIconProps={{ children: <IconDotsVertical /> }}
            modalProps={{
                size: '80%',
                title: 'Chọn thành viên phụ trách'
            }}
        >
            <CustomFieldset title="Danh sách người dùng">
                <CustomDataTable
                    isLoading={userQuery.isLoading}
                    isError={userQuery.isError}
                    columns={columns}
                    data={processedData}
                    enableColumnFilters
                    enableGlobalFilter
                    enableSorting
                    initialState={{
                        pagination: { pageSize: 10, pageIndex: 0 },
                    }}
                    renderRowActions={({ row }) => {
                        return (
                            <CustomCenterFull>
                                <CustomButton
                                    leftSection={<IconSelect />}
                                    color='violet'

                                    actionType='update'
                                    onClick={() => handleSelect(row)}
                                >Chọn</CustomButton>
                            </CustomCenterFull>
                        );
                    }}
                />
            </CustomFieldset>
        </CustomButtonModal>
    );
}
