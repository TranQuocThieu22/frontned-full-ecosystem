...
export default function CompletedTaskSubmitButton({ values }: Props) {
    ...
    const handleSubmit = async () => {
        ...
        queryClient.invalidateQueries({ queryKey: ["PeriodicQualityImprovementReportTracking", filterStore.state.Phase?.id] });
        disc[1].close();
        notifications.show({
            color: "green",
            message: "Cập nhật kết quả cải tiến thành công"
        });
    }
        ...
        >
            <Stack align="center" gap="md">
                ...
                >
                    {
                        values?.code
                            ? `Bạn sắp nộp báo cáo kết quả cải tiến của ${values?.eaqTaskDetail?.code || ""}. Bạn có chắc chắn không?`
                            : "Bạn sắp nộp báo cáo kết quả cải tiến. Bạn có chắc chắn không?"
                    }
                </Highlight>
