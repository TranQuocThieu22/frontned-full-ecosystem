...
import ActionPlanLayout from "./ViewDetail/ActionPlanLayout";
import CurrentSituationLayout from "./ViewDetail/CurrentSituationLayout";
import SelfEvaluationLayout from "./ViewDetail/SelfEvaluationLayout";
import StrengthsLayout from "./ViewDetail/StrengthsLayout";
import WeaknessesLayout from "./ViewDetail/WeaknessesLayout";
...
export default function SelfAssessmentViewDetail({ phaseId, taskDetailId }: Props) {
    const disclosure = useDisclosure();
    const [fullscreen, setFullScreen] = useState(false);
    const [isCollapsed, setIsCollapsed] = useState(false);
    const containerRef = useRef<HTMLDivElement>(null);
    useIsFetching({ queryKey: ["Self_Assessment_Type1_List"] }) // Tác dụng rerender để observer của thằng TableOfContents nó bắt được scroll, render lần đầu nó không bắt được scrollHost
