...
export default function EvidenceTable({ handleUse, data, isUsed }: EvidenceTableProps) {
  ...
  return (
    <CustomDataTable
      ...
      renderRowActions={({ row }) => {
        return (
          <CustomCenterFull>
            <Form04CurrentSituationDetail evidence={row.original} evidenceId={row.original.id!} />
            <CustomActionIcon
              onClick={() => {
                handleUse(
                  isUsed ? row.original?.eaqEvidenceId ?? 0 : row.original?.id ?? 0,
                  row.original.code || "",
