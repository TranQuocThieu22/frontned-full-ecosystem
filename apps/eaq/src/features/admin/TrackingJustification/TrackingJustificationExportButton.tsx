...
export default function TrackingJustificationExportButton({ table, loading }: Props) {
  ...
  const exportConfig = {
    fields: [
      ...
      },
      {
        ...
        formatFunction: (value: any, row: IRequirement) => row?.eaqCriteria?.code || "",
      },
      {
        header: "Mã yêu cầu",
        fieldName: "code",
      ...
      },
      {
        ...
        formatFunction: (value: any, row: IRequirement) => row?.hostUnit?.name || "",
      },
      {
        header: "Nhân sự phụ trách",
        fieldName: "userFullName",
        formatFunction: (value: any, row: IRequirement) => row?.user?.fullName || "",
      },
      {
        header: "Đã nộp",
        fieldName: "submitted",
        formatFunction: (value: any, row: IRequirement) =>
          row?.reportStatus === TaskDetailReportStatusEnum.IsSubmitted ? "Đã nộp" : "Chưa nộp",
      },
      {
        header: "Trạng thái tổng hợp",
        fieldName: "reportStatus",
        formatFunction: (value: any, row: IRequirement) =>
          row?.reportStatus !== undefined ? ReportStatusEnumLabel[row.reportStatus as TaskDetailReportStatusEnum] || "" : "",
      },
      {
        header: "Trạng thái kiểm tra",
        fieldName: "trackingStatus",
        formatFunction: (value: any, row: IRequirement) =>
          row?.trackingStatus !== undefined ? TrackingStatusEnumLabel[row.trackingStatus as keyof typeof TrackingStatusEnumLabel] || "" : "",
      },
    ],
  };
