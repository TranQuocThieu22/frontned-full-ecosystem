...
export default function TrackingJustificationViewButton({ data }: { data: IRequirement }) {
  ...
  const [value, setValue] = useState<string>(requirementDetailQuery.data?.requirementReport ?? "");
  useEffect(() => {
    setValue(requirementDetailQuery.data?.requirementReport ?? "");
  }, [requirementDetailQuery.data?.requirementReport]);
