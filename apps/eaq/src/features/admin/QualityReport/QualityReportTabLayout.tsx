...
export function QualityReportTabLayout() {
    const filterStore = useS_Shared_Filter();

    const periodicReportQuery = useCustomReactQuery({
        queryKey: ["QualityReportTracking", filterStore.state.Phase?.id],
        axiosFn: () => {
            return service_EAQAnalysis.getEAQTaskDetailReportsByEAQPhaseId({ eaqPhaseId: filterStore.state.Phase?.id, analysisType: analysisTypeEnum.Requirement });
        },
        options: {
            ...
            tabs={[
                {
                    ...
                    children: <CompletedTaskTable qualityReportQuery={periodicReportQuery} />,
                    leftSection: <IconCheck />
                },
                {
                    label: "Danh sách minh chứng được thu thập",
                    children: <CollectedEvidenceTable qualityReportQuery={periodicReportQuery} />,
