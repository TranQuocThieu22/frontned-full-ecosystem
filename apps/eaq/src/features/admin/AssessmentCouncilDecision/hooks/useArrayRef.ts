...
export function useArrayRef<T>(initialItems?: T[]) {
    ...
    const cloneItemsTranform = (transform: (item: T) => T, arrConcat?: T[]): T[] => {
        const valueToReturn: T[] = []
        for (const item of arrayRef.current) {
            valueToReturn.push(transform(structuredClone(item)));
        }
        if (arrConcat) {
            for (const item of arrConcat) {
                valueToReturn.push(transform(structuredClone(item)));
    ...
    /**
     * Convert a list of items (stored in a ref) into another type of array
     * using a transform function.
     *
     * @template V - Output item type
     * @param transform - Function to convert each item into the desired type
     * @returns Transformed array
     *
     * @example
     * const items = toComboboxItem((u) => ({
     *   value: String(u.id),
     *   label: u.name,
     * }));
     */
    function mapTo<V>(transform: (item: T) => V): V[] {
        return arrayRef.current.map((i) => transform(i));
    }

    /**
     * Projects each element of the array (stored in a ref) into a new form and flattens the result.
     *
     * @template V - The type of elements in the resulting array.
     * @param transform - A mapping function to transform each item of type `T` into type `V`.
     * @returns A flat array of transformed items.
     *
     * @example
     * // Convert users to combobox items
     * const rows = groups.flatMapTo(group =>
     ...
     * );
     */
    function flatMapTo<V>(transform: (item: T) => V | V[]): V[] {
        return arrayRef.current.flatMap((i) => transform(i));
    }
    ...
    /** - Check if array has at least one item that matches predicate.
     * - Example: const has = arrayRef.arrayHelpers.hasItem(arr, x => x.code === "A") */
    function hasItemHelper<V extends Record<string, any>>(arr: V[], predicate: (item: V) => boolean): boolean {
...
export interface IUseArrayRefController<T> {
     ...
     * - Useful when you need deep copy before transform.
     * - Example: arrayRef.cloneItemsTranform(x => ({...x, updated: true})) */
    cloneItemsTranform: (transform: (item: T) => T, arrConcat?: T[]) => T[];
     ...
     * );
     */
    flatMapTo: <V>(transform: (item: T) => V | V[]) => V[];
    ...
    arrayHelpers: {
        ...
        /** - Check if array has at least one item that matches predicate.
     * - Example: const has = arrayRef.arrayHelpers.hasItem(arr, x => x.code === "A") */
        hasItem: <V extends Record<string, any>>(arr: V[], predicate: (item: V) => boolean) => boolean
    }
}
