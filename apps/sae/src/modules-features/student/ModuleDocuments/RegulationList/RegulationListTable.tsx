...
export default function RegulationListTable() {
  const Q_Document = useCustomReactQuery({
    queryKey: ["document.GetByType"],
    axiosFn: () => service_document.getByType(KEYVALUE_DOCUMENT_TYPES.Regulations)
  })
  ...
  return (
    <CustomFieldset title="Danh mục quy định">
      <CustomFlexColumn>
        <Accordion
...
function DocumentSubTable({ attributeName, documents, attributeId }: { attributeName: string, documents: Document[], attributeId: number }) {
  const columns = useMemo<MRT_ColumnDef<Document>[]>(() => [
    { accessorKey: "decisionCode", header: "Số quy định", enableSorting: true },
    {
      ...
      Cell: ({ row }) => (
        <Text>
          {row.original.promulgateDate
            ? dateUtils.toDDMMYYYY(new Date(row.original.promulgateDate))
            : "N/A"}
        </Text>
      ),
    },
    { accessorKey: "name", header: "Tên tài liệu", enableSorting: true },
    {
      ...
      Cell: ({ row }) => (
        <CustomCenterFull>
          <CustomCheckbox
            checked={row.original.isCycleCheck}
            readOnly
          />
        </CustomCenterFull>
      ),
