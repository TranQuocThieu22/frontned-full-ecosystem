...
export default function YouthUnionButtonUpdate({ eventValue, iconType = "number", reviewedByUserId, userWorkingUnitId, userRoleIds }: { eventValue: Event, iconType?: IconType, reviewedByUserId: number | null, userWorkingUnitId: number | null, userRoleIds: number[] | null }) {
    const [fileData, setFileData] = useState<any[]>([]);
    const disc = useDisclosure();
    const permissionStore = usePermissionStore()
    const currentLoginUser = useAuthenticateStore()
    const studentEventQuery = useCustomReactQuery({
        queryKey: ["YouthUnionButtonUpdate", eventValue.id],
    ...
    return (
        <Group>
            <CustomButtonModal
                ...
                //todo ktra lại hàm onsubmit, custombuttonModal ko có
                            ...
                            <Grid.Col span={{ base: 12, md: 2 }}>
                                <Flex direction="column" justify="flex-end" align="center" h={60}>
                                    <CustomButton
                                        ...
                                        onClick={handleSubmit} leftSection={<IconPlus />} color='blue'>
                                        Thêm
                                    </CustomButton>
                                </Flex>
                            </Grid.Col>
