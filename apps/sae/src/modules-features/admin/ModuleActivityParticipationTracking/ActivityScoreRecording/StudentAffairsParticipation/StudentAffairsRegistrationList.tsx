...
import StudentAffairsRegistrationDelete from './StudentAffairsRegistrationDelete';

type IconType = "number" | "icon";

export default function StudentAffairsRegistrationList({ eventValue, iconType = "number", reviewedByUserId, userWorkingUnitId, userRoleIds }:
  { eventValue: Event, iconType?: IconType, reviewedByUserId: number | null, userWorkingUnitId: number | null, userRoleIds: number[] | null }) {
  ...
  const studentEventQuery = useCustomReactQuery({
    queryKey: ["YouthUnionRegistrationList_GetByEvent", eventValue.id],
              ...
              <CustomCenterFull>
                {(permissionStore.state.currentPermissionPage?.isDelete) &&
                  ((reviewedByUserId !== null && userWorkingUnitId !== null &&
                    reviewedByUserId === userWorkingUnitId) || (userRoleIds?.some(item => item === 2)) || currentLoginUser.state.userId?.toString() === '1') ?
                  <>
                    <StudentAffairsRegistrationDelete id={row.original.id!} name={row.original.studentName!} />
                  </>
                  :
                  <></>
                }
