...
import { RegisterType } from "../TabGeneralInfo/ActivityPlanInfo";
...
export default function RegisterObject({
    ...
    const classQuery = useCustomReactQuery({
        queryKey: ["classQuery", activityPlanStore.state.ActivityPlan?.id],
    ...
    const studentQuery = useCustomReactQuery({
        queryKey: ["studentQuery", activityPlanStore.state.ActivityPlan?.id],
        axiosFn: () => {
            return service_studentActivityPlan.findbyActivityPlan({
                activityPlanId: activityPlanStore.state.ActivityPlan?.id ?? 0,
            })
        },
        options: {
