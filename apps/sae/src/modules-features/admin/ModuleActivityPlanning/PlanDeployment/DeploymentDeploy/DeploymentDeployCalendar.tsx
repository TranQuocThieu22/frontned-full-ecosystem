...
import { Card, Flex, Modal, Skeleton, useMantineColorScheme, Text } from "@mantine/core";
...
export default function DeploymentDeployCalendar({ data, isRequiredHidden, skeleton }: CalendarProps) {
    const { colorScheme } = useMantineColorScheme();
    const [selectedEvent, setSelectedEvent] = useState<Event | null>(null);
    const disclosure = useDisclosure(false);

    const [moreLinkEvents, setMoreLinkEvents] = useState<Event[]>([]);
    const moreLinkDisclosure = useDisclosure(false);
    ...
    useEffect(() => {
        if (!selectedEvent || !disclosure[0]) return;
        ...
        <>
            ...
            >
                <FullCalendar
                    ...
                    moreLinkClick={(arg) => {
                        const dayEvents = arg.allSegs.map((seg) => seg.event.extendedProps.rawData as Event);
                        setMoreLinkEvents(dayEvents);
                        moreLinkDisclosure[1].open();
            ...
            />
            <Modal
                opened={moreLinkDisclosure[0]}
                onClose={moreLinkDisclosure[1].close}
                title={
                    <Text fw={600}>
                        {moreLinkEvents[0]?.startDate
                            ? `Hoạt động ngày ${new Date(moreLinkEvents[0].startDate).toLocaleDateString("vi-VN")}`
                            : "Danh sách hoạt động"}
                    </Text>
                }
                size="lg"
            >
                <Flex direction="column" gap="sm">
                    {moreLinkEvents.map((event) => {
                        const cleanTitle = event.name?.replace(/<[^>]*>/g, "") || "Không có tên";
                        const start = event.startDate ? new Date(event.startDate).toLocaleDateString("vi-VN") : "";
                        const end = event.endDate ? new Date(event.endDate).toLocaleDateString("vi-VN") : "";
                        ...
                        return (
                            <Card
                                ...
                                onClick={() => {
                                    ...
                                    moreLinkDisclosure[1].close();
                                }}
                            >
                                <Text fw={500} size="sm" lineClamp={2}>{cleanTitle}</Text>
                                <Text size="xs" c="dimmed" mt={4}>{start} – {end}</Text>
                            </Card>
                        );
                    })}
                </Flex>
