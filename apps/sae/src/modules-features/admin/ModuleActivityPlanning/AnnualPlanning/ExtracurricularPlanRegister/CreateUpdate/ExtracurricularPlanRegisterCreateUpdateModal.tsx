...
export default function ExtracurricularPlanRegisterCreateUpdateModal({ futurePlanId, values, loadingActionIcon }: Props) {
  ...
  const form = useForm<Event>({
    initialValues: {
      ...
      startDate: values?.startDate
        ? (typeof values.startDate === 'string' && values.startDate.includes('T')
          ? values.startDate
          : `${new Date(values.startDate).toLocaleDateString('en-CA')}T07:00:00`)
        : `${new Date().toLocaleDateString('en-CA')}T07:00:00`,
      endDate: values?.endDate
        ? (typeof values.endDate === 'string' && values.endDate.includes('T')
          ? values.endDate
  ...
  const createFutureEventMutation = useMutation({
    mutationFn: async (payload: any) => {
      let res = await service_event.ExternalCreateFutureEvent(payload)
            ...
            tabs={[
              {
                ...
                children: <ExtracurricularPlanInfo
                  disclosure={disc}
                  form={form}
                  isUpdate={isUpdate}
                />
