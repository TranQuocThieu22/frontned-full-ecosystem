...
export default function ExtracurricularPlanRegisterImport() {
  ...
  const importMutation = useCustomReactMutation({
    ...
    axiosFn: async (payload: {
      ...
      const eventResults = await Promise.allSettled(
        eventsWithPlan.map((event) => service_event.ExternalCreateFutureEvent(event))
      );
      ...
      // Show custom notifications based on results
      if (totalFailures > 0) {
        if (totalSuccesses > 0) {
          // Partial success - show warning
          notifications.show({
            title: "Import hoàn thành với một số lỗi",
            message: `Thành công: ${totalSuccesses}/${eventsWithPlan.length} hoạt động. ${totalFailures} hoạt động thất bại.`,
            color: "yellow",
            autoClose: 5000,
          });
        } else {
          // Complete failure - show error notification instead of throwing
          notifications.show({
            title: "Import thất bại",
            message: `Import thất bại. ${totalFailures} hoạt động không thể import được.`,
            color: "red",
            autoClose: 5000,
          });
        }
      } else {
        // Complete success - show success notification
        notifications.show({
  ...
  const handleExport = async () => {
    try {
      ...
      excelUtils.download({ name: "Đăng ký kế hoạch tổ chức hoạt động ngoại khóa", workbook });
    } catch (error) {
  ...
  const handleExecute = async (finalValues: any[]) => {
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ["ExternalFuturePlan"] });
      stack.closeAll();
    } catch (error) {
      console.error(error);
      notifications.show({
