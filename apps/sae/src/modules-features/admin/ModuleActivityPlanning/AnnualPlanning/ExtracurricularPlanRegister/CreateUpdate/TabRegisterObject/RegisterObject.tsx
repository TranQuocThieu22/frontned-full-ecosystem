...
import { RegisterType } from "../TabGeneralInfo/ExtracurricularPlanInfo";
...
export default function RegisterObject({
    ...
    const classColumns = useMemo<MRT_ColumnDef<any>[]>(() => [
        ...
        {
            header: "Mã khoa",
            accessorFn: (row) => row.majors?.faculty ? row.majors?.faculty?.code : "",
            size: 150
        },
        {
            header: "Mã ngành",
            accessorFn: (row) => row.majors ? row.majors.code : "",
            size: 150
        },
    ], []);
