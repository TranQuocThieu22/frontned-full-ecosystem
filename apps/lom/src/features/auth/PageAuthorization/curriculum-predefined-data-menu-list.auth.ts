...
export const canViewAssignCurriculumpPredefinedDataMenuList = (userStore?: IStore_Authenticate | null, userPermissionsStore?: PermissionStoreProps | null) => {
    if (hasAllPermissions(userStore, userPermissionsStore)) return true
    if (!!userPermissionsStore?.permission?.find(permission => permission.pageId === AppPage.CurriculumpPredefinedDataMenuList)?.isRead) return true
    return false
};

export const canViewAssignCurriculumpPredefinedDataMenuListItem = (userStore?: IStore_Authenticate | null, userPermissionsStore?: PermissionStoreProps | null, pageId?: number) => {
    if (hasAllPermissions(userStore, userPermissionsStore)) return true
