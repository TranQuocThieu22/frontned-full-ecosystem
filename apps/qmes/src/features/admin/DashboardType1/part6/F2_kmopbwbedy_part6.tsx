...
export default function F2_kmopbwbedy_part1() {
  ...
  const [opened61, set61Opened] = useState(false);
  const [opened6211, set6211Opened] = useState(false);
  const [opened6212, set6212Opened] = useState(false);
  const [opened6221, set6221Opened] = useState(false);
  const [opened6222, set6222Opened] = useState(false);

  const [openedChart61, handlers61] = useDisclosure(false);
  const [openedChart6211, handlers6211] = useDisclosure(false);
  const [openedChart6212, handlers6212] = useDisclosure(false);
  const [openedChart6221, handlers6221] = useDisclosure(false);
  const [openedChart6222, handlers6222] = useDisclosure(false);
    ...
    <>
      ...
      <MyFlexRow>
        <Group>
          ...
          <IconStethoscope color="blue" />

          <Text size="xl">6. Nghiên cứu và đổi mới sáng tạo</Text>
