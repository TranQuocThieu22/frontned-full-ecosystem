...
import { IQuestionCatalogMCQSingleViewModel } from "./interfaces/QuestionCatalogViewModel";

const DEFAULT_ANSWER: IQuestionCatalogMCQSingleViewModel = {
    text: "",
    isCorrect: false,
    weight: "0%",
    analysis: ""
...
export default function QuestionCatalogMCQSingle({ data }: { data?: IQuestionCatalogMCQSingleViewModel[] }) {
    const [answers, setAnswers] = useState<IQuestionCatalogMCQSingleViewModel[]>([]);
    const [newAnswer, setNewAnswer] = useState<IQuestionCatalogMCQSingleViewModel>(DEFAULT_ANSWER);
    ...
    const handleChangeAnswer = (index: number, field: keyof IQuestionCatalogMCQSingleViewModel, value: string | boolean) => {
    ...
    const handleChangeNewAnswer = (field: keyof IQuestionCatalogMCQSingleViewModel, value: string | boolean) => {
    ...
    const renderAnswerRow = (answer: IQuestionCatalogMCQSingleViewModel, index: number) => (
        <Table.Tr key={index}>
            <Table.Td>
                <MyTextInput
