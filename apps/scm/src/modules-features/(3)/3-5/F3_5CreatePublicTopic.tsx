...
export default function F3_5CreatePublicTopic() {
    const ParticipatingMembers = useListState<IParticipatingMembers>()
    ...
    const columns = useMemo<MRT_ColumnDef<IParticipatingMembers>[]>(
        () => [
            ...
            },
            {
                header: "Vai trò",
                accessorFn: (row) => {
                    return <MySelect data={RoleData.map(item => ({ value: item.id.toString(), label: item.name }))} onChange={(value) => ParticipatingMembers[1].applyWhere((item) => item.id == row.id, (item) => ({ ...item, roleId: parseInt(value?.toString()!) }))} />
                }
            },
        ],
        []
    ...
    return (
        <MyButtonCreate onSubmit={() => { }} form={form} modalSize={"80%"}>
            <TextInput
                label="Mã đề tài"
