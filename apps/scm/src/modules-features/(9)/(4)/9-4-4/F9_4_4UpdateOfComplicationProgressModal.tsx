...
import { dataMockPhaseStatus } from "./mockData";
export default function F9_4_4UpdateOfComplicationProgressModal({values}: { values: IF9_4_4UpdateComplicationProgressButtonModal }) {
    const disc = useDisclosure();
    const form = useForm<IF9_4_4UpdateComplicationProgressButtonModal>({
        initialValues: {
            ...values
        },
        validate: {
            ...
            phaseStatus: (value) => (value ? null : "Trạng thái giai đoạn không được để trống"),
            actualEndDate: (value) => (value ? null : "Ngày kết thúc thực tế không được để trống"),
            progressStatus: (value) => (value ? null : "Trạng thái tiến độ không được để trống"),
    ...
    return(
        <MyButtonModal label="Cập nhật" title="Chi tiết công việc" color="blue" modalSize={"70%"} disclosure={disc}>
            <Grid>
            <Grid.Col span={6}>
                <MySelect label="Trạng thái giai đoạn" data={dataMockPhaseStatus} {...form.getInputProps("phaseStatus")}></MySelect>
