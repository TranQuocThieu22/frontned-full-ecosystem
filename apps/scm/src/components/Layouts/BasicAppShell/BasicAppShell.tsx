...
function getRightSection(status: string) {
    ...
    if (status === "New") return <Badge styles={{ root: { cursor: "pointer" } }} radius="xs">N</Badge>
    if (status === "Menu") return <Badge styles={{ root: { cursor: "pointer" } }} radius="xs" color="orange">M</Badge>;;
...
function generateLinkEnum(prefix: string, data: I0LinkItem[]): { [key: string]: number } {
    ...
    const traverseItems = (items: I0LinkItem[]) => {
        items.forEach(item => {
            if (item.label == "1. Quản lý hệ thống") return
            if (item.label == "2. Văn bản - Quy định") return

            if (item.label == "11.3 Danh mục phân loại văn bản") return
            processItem(item);
...
export function BasicAppShell({ children, menu }: { children: ReactNode, menu: I0LinkItem[] }) {
    ...
    useEffect(() => {
        undefinedMenu(menu);
        checkUniqueIds(menu);
        console.log(generateLinkEnum("SRM", menu));
    }, [])
    return (
        <AppShell
            header={{ height: 60 }}
