    ...
    items.forEach(item => {
        ...
        if (item.link) {
            actions.push({
                ...
                label: item.label,
                description: `Chuyển đến ${item.label}`,
                onClick: () => {
        ...
        if (item.links) {
            actions = actions.concat(
                convertMenuToSpotlightActions(item.links, router, item.link || '', sideBarStore)
            );
        }
    });
...
export function MyAppSpotlight({ menu }: { menu: I0LinkItem[] }) {
    ...
    const spotlightActions = useMemo(() =>
        convertMenuToSpotlightActions(menu, router, '', sideBarStore),
