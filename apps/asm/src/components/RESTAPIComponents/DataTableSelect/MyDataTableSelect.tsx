...
export default function MyDataTableSelect<TData extends MRT_RowData>({ exportAble, modalSize, renderTopToolbarCustomActions, data, selectButtonlabel, listState, columns, listLabel, ...rest }: IDataTable<TData>) {
                ...
                renderTopToolbarCustomActions={({ table }) => {
                    return (
                        <Group>
                            ...
                            <Button leftSection={<IconPlus />} onClick={disc[1].open}>{selectButtonlabel || "Thêm"}</Button>
                        </Group>
                    )
                }}
                columns={columns}
                ...
                <MyDataTable
                    renderTopToolbarCustomActions={({ table }) => {
                        return <Button onClick={() => {
                            const existingItems = listState[0];
                            ...
                            selectedItems.forEach(item => listState[1].append(item));
                            disc[1].close();
                        }}>Chọn</Button>
                    }}
                    enableRowSelection
