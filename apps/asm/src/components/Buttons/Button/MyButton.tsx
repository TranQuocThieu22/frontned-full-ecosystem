...
import { IconDeviceFloppy, IconEdit, IconFileImport, IconPlus, IconPrinter, IconTrash } from "@tabler/icons-react";
...
export function MyButton({ children, crudType = "delete", ...rest }: IMyButton) {
    ...
    if (crudType == "add") {
        return (
            <Button leftSection={<IconPlus />} {...rest}>
                {children ? children : "Thêm"}
            </Button>
        );
    }
