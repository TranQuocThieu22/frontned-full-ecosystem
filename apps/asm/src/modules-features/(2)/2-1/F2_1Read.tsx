...
import F2_1Delete from "./F2_1Delete";
import F2_1Update from "./F2_1Update";
interface I {
    documentType?: number;
    id?: number;
    code?: string;
...
export default function F2_1Read() {
    const query = useQuery<I[]>({
        queryKey: ['F2_1Read', C0DocumentTypes.Regulations],
        queryFn: async () => {
            const result = await baseAxios.get("/DocumentAttribute/GetByType?", { params: { documentType: C0DocumentTypes.Regulations } })
            ...
            <Accordion.Panel>
                <MyDataTable
                    ...
                    renderRowActions={({ row }) => (
                        <MyCenterFull>
                            <F2_1Update values={row.original!} />
                            <F2_1Delete id={row.original.id!} />
                        </MyCenterFull>
                    )}
                />
            </Accordion.Panel>
