...
import { IF12_2_1ReadNewCurriculum, IF12_2_1ReadNewCurriculumTable, mockDataCurriculumTable } from "./F12_2_1ReadNewCurriculum";
import { mockDataFieldOfStudy, mockDataMajorApplied } from "./mockData";
export default function F12_2_UpdateNewCurriculum({ values }: { values: IF12_2_1ReadNewCurriculum }) {
    const query = useQuery<IF12_2_1ReadNewCurriculumTable[]>({
        queryKey: ['query'],
        queryFn: () => {
            return mockDataCurriculumTable ?? [];
        }
    });
    ...
    return (
        <MyActionIconUpdate title="Chi tiết đăng ký đề xuất" modalSize={"80%"} form={form} onSubmit={(values) => console.log(values)}>
                    ...
                    <Grid>
                        <Grid.Col span={6}>
                            ...
                            <MySelect
                                ...
                                defaultValue={JSON.stringify(values.fieldOfStudy)}
                                pt={10}
                            />
