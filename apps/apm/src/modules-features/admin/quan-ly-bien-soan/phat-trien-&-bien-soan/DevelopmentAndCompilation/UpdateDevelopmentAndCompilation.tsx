...
import { MyActionIconUpdate, MyCheckbox, MyDateInput, MyFileInput, MySelect, MyTextArea, MyTextInput, } from "aq-fe-framework/components";
...
export default function DevelopmentAndCompilationUpdate({ values }: { values: DevelopmentAndCompilation }) {

    const form = useForm<DevelopmentAndCompilation>({
        initialValues: values
    })
    return (
        <MyActionIconUpdate modalSize={"60%"} title="Chi tiết giao nộp" form={form} onSubmit={() => { }}>
            <SimpleGrid cols={2}>
                <MyTextInput
                    label="Ban Biên soạn"
