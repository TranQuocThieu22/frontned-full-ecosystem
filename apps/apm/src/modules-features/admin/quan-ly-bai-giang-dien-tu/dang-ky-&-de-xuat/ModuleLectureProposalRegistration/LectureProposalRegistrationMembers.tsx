...
import { ILectureProposalRegistrationMemberViewModel, ILectureProposalRegistrationUserViewModel } from "./interfaces/ILectureProposalRegistrationViewModel"


export default function LectureProposalRegistrationMembers({ data }: { data?: ILectureProposalRegistrationMemberViewModel[] }) {
    const [members, setMembers] = useState<ILectureProposalRegistrationMemberViewModel[]>([...(data || [])])
    ...
    const handleAddMember = () => {
        if (mockMember.length > 0) {
            const newMember: ILectureProposalRegistrationMemberViewModel = {
                ...mockMember[0],
                role: mockRole[1] || '',
            }
            ...
            Cell: ({ row }) => {
                ...
                return (
                    <MySelect
                        ...
                        data={mockMember.map(item => ({ value: String(item.id), label: item.code || "" })) || []}
                    />
                )
            }
        },
...
export const mockMember: ILectureProposalRegistrationUserViewModel[] = [
    {
        id: 1,
        name: "Tô Ngọc Bảo",
        code: "GV0258",
