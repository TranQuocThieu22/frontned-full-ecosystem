...
export default function PaymentSettlementTable() {
    ...
    const columns = useMemo<MRT_ColumnDef<I_PaymentSettlement>[]>(() => [
        ...
        { header: 'Ngày quyết định/Ngày ký duyệt', accessorKey: 'decisionDate', accessorFn: (row) => row.decisionDate ? utils_date.toDDMMYYYY(new Date(row.decisionDate)) : "" },
        ...
        },
        {
            ...
            accessorFn: (row) =>
                row.modifiedAt ? utils_date.toDDMMYYYY(new Date(row.modifiedAt)) : "",
        },
    ], []);
