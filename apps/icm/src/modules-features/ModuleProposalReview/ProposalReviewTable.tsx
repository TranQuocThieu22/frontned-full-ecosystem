import { Flex } from "@mantine/core";
import { useQuery } from "@tanstack/react-query";
import { MyButton, MyCenterFull, MyDataTable, MyFieldset } from "aq-fe-framework/components";
import { MRT_ColumnDef } from "mantine-react-table";
import { useMemo } from "react";
import { ProposalReviewViewModel } from "./interfaces/ProposalReviewViewModel";
import ProposalReviewApprove from "./ProposalReviewApprove";
import ProposalReviewView from "./ProposalReviewView";

export const EnumProposalReviewStatus = {
    "1": "Chờ sơ duyệt",
    "2": "Yêu cầu bổ sung",
    "3": "Đã sơ duyệt - Chờ hội đồng",
    "4": "Đã phê duyệt",
    "5": "Từ chối sơ bộ"
};
export default function ProposalReviewTable() {
    const Query = useQuery({
        queryKey: ["lecture-proposal-registration"],
        queryFn: () => {
            return mockData;
        },
    })

    const columns = useMemo<MRT_ColumnDef<ProposalReviewViewModel>[]>(() => [
        {
            header: "Tên bài giảng",
            accessorKey: "name",
        },
        {
            header: "Mã bài giảng",
            accessorKey: "code",
        },
        {
            header: "Lĩnh vực Khoa học",
            accessorFn: (row) => {
                return row.scienceFields.map((field) => field.name).join(", ")
            },
            size: 200,
        },
        {
            header: "Người phụ trách chính",
            accessorKey: "responsiblePerson",
            accessorFn: (row) => `${row.responsiblePerson.name}`
        },
        {
            header: "Thời gian bắt đầu dự kiến",
            accessorKey: "startDate",
        },
        {
            header: "Thời gian hoàn thành dự kiến",
            accessorKey: "endDate",
        },
        {
            header: "Dự kiến kinh phí",
            accessorKey: "budget",
            accessorFn: (row) => {
                return `${row.budget.toLocaleString()} VNĐ`
            }
        },
        {
            header: "Ghi chú kiểm tra",
            accessorKey: "note",
        },
        {
            header: "Trạng thái đăng ký",
            accessorKey: "status",
            accessorFn: (row) => {
                return EnumProposalReviewStatus[row.status as keyof typeof EnumProposalReviewStatus]
            }
        },
    ], []);

    return (
        <MyFieldset
            title="Danh sách đăng ký"
        >
            <MyDataTable
                columns={columns}
                data={Query.data || []}
                renderTopToolbarCustomActions={({ table }) => {
                    return (
                        <>
                            <MyButton crudType="export" />
                        </>
                    )
                }}
                renderRowActions={({ row }) => {
                    return (
                        <MyCenterFull >
                            <Flex gap="10">
                                <ProposalReviewView data={row.original} />
                                <ProposalReviewApprove status={row.original.status} />
                            </Flex>
                        </MyCenterFull>
                    )
                }}
            />
        </MyFieldset>
    );
}

const mockData: ProposalReviewViewModel[] = [
    {
        id: 1,
        name: "Lập trình Python cơ bản",
        code: "PYB-2025-001",
        scienceFieldCodes: ["CNTT"],
        scienceFields: [
            {
                id: 1,
                name: "Công nghệ thông tin",
                code: "CNTT",
            }
        ],
        objective: "Giúp sinh viên nắm vững cú pháp và lập trình hướng đối tượng; Áp dụng Python giải quyết bài toán cơ bản",
        targetAudience: "Sinh viên năm nhất ngành CNTT",
        responsiblePerson: {
            id: 1,
            code: "12345",
            name: "Nguyễn Văn A",
        },
        members: [
            {
                id: 5,
                code: "67890",
                name: "Nguyễn Thị B",
                unit: "KCNTT",
                role: "Thành viên",
            },
            {
                id: 6,
                code: "11223",
                name: "Trần Văn C",
                unit: "KDDT",
                role: "Thành viên",
            },
        ],
        startDate: "2025-09-01",
        endDate: "2025-02-28",
        budget: 50000000,
        requirements: "Cần nền tảng LMS; Hỗ trợ thiết bị ghi âm",
        status: "1",
        summary: "Gồm 6 module; Giới thiệu Python; Cấu trúc dữ liệu; Lập trình hướng đối tượng; Xử lý file; Các thư viện cơ bản; Bài tập thực hành",
        filePath: "quyet_dinh_14_2024.pdf",
        fileDetail: {
            fileName: "quyet_dinh_14_2024.pdf",
            fileExtension: "pdf",
            fileBase64String: "",
        },
    },
    {
        id: 2,
        name: "Trí tuệ nhân tạo trong Y học",
        code: "AIH-2025-002",
        scienceFieldCodes: ["YHC", "AI"],
        scienceFields: [
            {
                id: 2,
                name: "Y học",
                code: "YHC",
            },
            {
                id: 3,
                name: "Trí tuệ nhân tạo",
                code: "AI",
            },
        ],
        objective: "Cung cấp kiến thức cơ bản về AI cho lĩnh vực y tế; Phân tích các ứng dụng AI trong chẩn đoán và điều trị",
        targetAudience: "Bác sĩ; Nghiên cứu sinh ngành y học",
        responsiblePerson: {
            id: 2,
            code: "54321",
            name: "PGS.TS. Trần Thị D",
        },
        members: [
            {
                id: 2,
                code: "98765",
                name: "TS. Phạm Quang E",
                unit: "KDDT",
                role: "Thành viên",
            },
        ],
        startDate: "2025-10-01",
        endDate: "2025-03-31",
        budget: 75000000,
        requirements: "Cần studio quay video; Phần mềm chỉnh sửa video chuyên dụng",
        status: "2",
        note: "Thiếu file thuyết minh chi tiết",
        summary: "Tổng quan về AI; Học máy; Học sâu; Ứng dụng AI trongn chuẩn đoán hình ảnh; Ứng dụng AI trong quản lý bệnh án",
        filePath: "quyet_dinh_14_2024.pdf",
        fileDetail: {
            fileName: "quyet_dinh_14_2024.pdf",
            fileExtension: "pdf",
            fileBase64String: "",
        },
    },
    {
        id: 3,
        name: "Quản lý dự án Agile cho NCKH",
        code: "AGILE-2025-003",
        scienceFieldCodes: ["KQT", "CNTT"],
        scienceFields: [
            {
                id: 1,
                name: "Công nghệ thông tin",
                code: "CNTT",
            },
            {
                id: 4,
                name: "Khoa học quản trị",
                code: "KQT",
            }
        ],
        objective: "Giúp sinh viên nắm vững các phương pháp quản lý dự án Agile; Áp dụng Agile vào quản lý dự án NCKH",
        targetAudience: "Giảng viên, Nghiên cứu viên",
        responsiblePerson: {
            id: 3,
            code: "23456",
            name: "TS. Hoàng Minh F",
        },
        members: [
        ],
        startDate: "2025-11-01",
        endDate: "2025-01-31",
        budget: 30000000,
        requirements: "Cần tài khoản công cụ quản lý dự án",
        status: "3",
        summary: "Giới thiệu về Agile; Scrum framwork; Kanban; Quản lý rủi ro trong Agile; Case study dự án NCKH",
        filePath: "quyet_dinh_14_2024.pdf",
    },
    {
        id: 4,
        name: "Blockchain ứngg dụng trong giáo dục",
        code: "BCE-2025-001",
        scienceFieldCodes: ["CNTT"],
        scienceFields: [
            {
                id: 1,
                name: "Công nghệ thông tin",
                code: "CNTT",
            }
        ],
        objective: "Giúp sinh viên nắm vững các khái niệm và ứng dụng của Blockchain trong giáo dục",
        targetAudience: "Giảng viên, Nghiên cứu viên",
        responsiblePerson: {
            id: 4,
            code: "45678",
            name: "TS. Lê Hữu G",
        },
        members: [
        ],
        startDate: "2025-12-01",
        endDate: "2025-05-31",
        budget: 80000000,
        requirements: "Cần tài khoản công cụ quản lý dự án",
        status: "4",
        summary: "Giới thiệu về Blockchain; Ứng dụng Blockchain trong giáo dục; Case study dự án NCKH",
        filePath: "quyet_dinh_14_2024.pdf",
    },
    {
        id: 5,
        name: "Tâm lý học hành vi",
        code: "TLH-2025-005",
        scienceFieldCodes: ["TLH"],
        scienceFields: [
            {
                id: 7,
                name: "Tâm lý học",
                code: "TLH",
            }
        ],
        objective: "Giúp sinh viên nắm vững các khái niệm và ứng dụng của Blockchain trong giáo dục",
        targetAudience: "Giảng viên, Nghiên cứu viên",
        responsiblePerson: {
            id: 5,
            code: "56789",
            name: "GV. Nguyễn Thị H",
        },
        members: [
        ],
        startDate: "2025-01-15",
        endDate: "2025-07-30",
        budget: 40000000,
        requirements: "Cần tài khoản công cụ quản lý dự án",
        status: "5",
        note: "Nội dung đề xuất không phù hợp định hướng đào tạo",
        summary: "Giới thiệu về Tâm lý học hành vi; Ứng dụng Tâm lý học hành vi trong giáo dục; Case study dự án NCKH",
        filePath: "quyet_dinh_14_2024.pdf",
    },
    {
        id: 6,
        name: "Lịch sử Văn học Việt Nam",
        code: "VLIT-2025-006",
        scienceFieldCodes: ["VH"],
        scienceFields: [
            {
                id: 8,
                name: "Văn học",
                code: "VH",
            }
        ],
        objective: "Giúp sinh viên nắm vững các khái niệm và ứng dụng của Blockchain trong giáo dục",
        targetAudience: "Giảng viên, Nghiên cứu viên",
        responsiblePerson: {
            id: 6,
            code: "67890",
            name: "GV. Nguyễn Văn K",
        },
        members: [
        ],
        startDate: "2025-09-15",
        endDate: "2025-03-15",
        budget: 45000000,
        requirements: "Cần tài khoản công cụ quản lý dự án",
        status: "1",
        summary: "Giới thiệu về Lịch sử Văn học Việt Nam; Ứng dụng Lịch sử Văn học Việt Nam trong giáo dục; Case study dự án NCKH",
        filePath: "quyet_dinh_14_2024.pdf",
        fileDetail: {
            fileName: "quyet_dinh_14_2024.pdf",
            fileExtension: "pdf",
            fileBase64String: "",
        },
    }
]