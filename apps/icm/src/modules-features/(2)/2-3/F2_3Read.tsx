...
import F2_3Delete from "./F2_3Delete";
import F2_3Update from "./F2_3Update";
interface I {
    documentType?: number;
    id?: number;
    code?: string;
...
export default function F2_3Read() {
    const query = useQuery<I[]>({
        queryKey: ['F2_3Read', C0DocumentTypes.Regulations],
        queryFn: async () => {
            const result = await baseAxios.get("/DocumentAttribute/GetByType?", { params: { documentType: C0DocumentTypes.Form } })
            ...
            <Accordion.Panel>
                <MyDataTable
                    ...
                    renderRowActions={({ row }) => (
                        <MyCenterFull>
                            <F2_3Update values={row.original!} />
                            <F2_3Delete id={row.original.id!} />
                        </MyCenterFull>
                    )}
                />
            </Accordion.Panel>
