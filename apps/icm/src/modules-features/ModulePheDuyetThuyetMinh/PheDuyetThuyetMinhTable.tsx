...
export default function PheDuyetThuyetMinhTable() {
    const columns = useMemo<MRT_ColumnDef<IThuyetMinhDeTaiInfoViewModel>[]>(() => [
        ...
        <MyFieldset title={"Danh sách thuyết minh đề tài"}>
            <MyDataTable
                ...
                renderRowActions={({ row }) => {
                    return (
                        <MyCenterFull>
                            <PheDuyetThuyetMinhApproveButton data={row.original} />
                        </MyCenterFull>
                    );
                }}
            />
...
const mockData: IThuyetMinhDeTaiInfoViewModel[] = [
    {
        id: 1,
