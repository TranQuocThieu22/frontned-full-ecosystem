...
export default function F_timeCluster_Form({ values }: { values?: IValues }) {
        ...
        form.setValues({
            ...
            danhSachBuoiHoc: values.timeClusterDetails?.map(item => {
                ...
                return ({
                    ...
                    soPhut: timeTypeDetailQuery.data![0]?.timeTypeDetails?.slice(item.classPeriodStart, item.classPeriodEnd! + 1).reduce((acc, cur) => acc + cur.minuteNumber!, 0),
    ...
    const addBuoiHoc = (thu: string) => {
        const newBuoiHoc: IBuoiHoc = {
            ...
            tietBatDau: 1,
            soTiet: 1,
            soPhut: timeTypeDetailQuery.data![0]?.timeTypeDetails![0]?.minuteNumber!,
        };
    ...
    const transformToTimeClusterDetails = (values: IBuoiHoc[]): ITimeClusterDetails[] => {
        ...
        const newValues = values.map(item => {
            ...
            return ({
                ...
                startTime: new Date(danhSachTietHoc?.find(tiet => tiet.order == item.tietBatDau)?.startHour! + "z"),
                endTime: utils_date_addMinutes(new Date(danhSachTietHoc?.find(tiet => tiet.order == item.tietBatDau + item.soTiet - 1)?.startHour! + "z"), danhSachTietHoc?.find(tiet => tiet.order == item.tietBatDau + item.soTiet - 1)?.minuteNumber!)
            }) as ITimeClusterDetails
        })
        return newValues
    }

    const handleSubmit = (values: I, isUpdate?: boolean) => {
        ...
        if (isUpdate) {
            updateMutation.mutate({
                id: values.id,
                code: values.maCumThoiGian,
                name: values.tenCumThoiGian,
                timeTypeId: 1,
                timeClusterDetails: transformToTimeClusterDetails(values.danhSachBuoiHoc!),
            }, {
                onSuccess: () => {
                    notifications.show({
                                                ...
                                                buoiHoc.thu === thu && (
                                                    <Group key={index} align="end" mb="xs">
                                                        ...
                                                        <MyNumberInput
                                                            ...
                                                            onChange={(e) => {
                                                                const soTiet = e
                                                                const soPhut = parseInt(soTiet.toString()) * timeTypeDetailQuery.data![0]?.timeTypeDetails?.find(item => item.order == form.getValues().danhSachBuoiHoc![index]?.tietBatDau)?.minuteNumber!
