...
import { service_courseSection } from '../../../api/services/service_courseSection';
...
export default function DayOffDetailQuery(
  ...
  const dayOffDetailQuery = useMyReactQuery({
    ...
    axiosFn: () => {
      ...
      return service_courseSection.ScheduleDetail({ param: PARAM })
    },
    options: {
  ...
  const lecturerQuery = useMyReactQuery({
    ...
    axiosFn: () => {
      const PARAM = `referenceId=${dayOffData.id}`
      return service_account.getAllLecturer()
    },
    options: {
      enabled: disc[0] != false,
      refetchOnWindowFocus: false
  ...
  useEffect(() => {
    if (dayOffDetailQuery.data?.courseSectionScheduleInfo) {
      setEditableData(prev => prev.filter(
        (item: any) =>
          dayOffData.id !== item.id
      ));
      setEditableData(structuredClone(dayOffDetailQuery.data.courseSectionScheduleInfo));
  ...
  const handleAddDayOff = () => {
    const filteredData = (dayOffDetailQuery.data?.courseSectionScheduleInfo || []).filter(
      (item: any) => dayOffData.id == item.id
    );
    const firstDayOff = Array.isArray(filteredData) ? filteredData[0] : filteredData;
    const newRow = {
    ...
    >
      {dayOffDetailQuery.isSuccess && <DayOffInfo
        dayOffDetailQuery={
          ...
          (dayOffDetailQuery.data?.courseSectionScheduleInfo || []).find(
            (item: any) => dayOffData.id == item.id
          ) || {}
        }
        ...
        (
          <MyFieldset title="Danh sách buổi dạy bù" >
            <MyDataTable
              columns={columns}
              data={
                (dayOffDetailQuery.data?.courseSectionScheduleInfo || []).filter(
                  (item: any) => dayOffData.id !== item.id
                )
              }
              renderTopToolbar={({ table }: any): any => (
                <>
