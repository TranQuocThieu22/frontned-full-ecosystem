...
export default function SelectDiscountSection() {
    ...
    const [allDiscountsType1, setAllDiscountsType1] = useState<IDiscountViewModel[]>([]);
    const [allDiscountsType2, setAllDiscountsType2] = useState<IDiscountViewModel[]>([]);
    ...
    const AllDiscounts = useQuery<IDiscountViewModel[]>({
        queryKey: [`AllDiscounts`],
        queryFn: async () => {
            const response = await baseAxios.get("/Discount/GetAll");
            if (response.data.isSuccess === 1) {
                setAllDiscountsType1(response.data.data.filter((item: IDiscountViewModel) => item.discountType === 1));
                setAllDiscountsType2(response.data.data.filter((item: IDiscountViewModel) => item.discountType === 2));
