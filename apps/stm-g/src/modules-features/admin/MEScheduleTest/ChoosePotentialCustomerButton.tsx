...
import { DisplayPotentialCustomerStatus, PotentialCustomerStatusMap } from "../MEListPotentialCustomer/DisplayPotentialCustomerStatus";
...
export default function ChoosePotentialCustomerButton({ defautValue }: { defautValue: any }) {
    ...
    const columns = useMemo<MRT_ColumnDef<PotentialStudent>[]>(
        () => [
            ...
            { accessorKey: 'source', header: 'Nguồn KHTN' },
            {
                ...
                accessorFn: (row) => <DisplayPotentialCustomerStatus customerStatus={PotentialCustomerStatusMap[row.status] ?? -1} />,
                size: 200
            },
            ...
            { accessorKey: 'lastUpdatedDate', header: 'Ngày cập nhật gần nhất', accessorFn: (row) => utils_date_dateToDDMMYYYString(new Date(row.createdDate)) },
        ], []
    );
    ...
    return (
        <>
            <TextInput
                ...
                readOnly
                placeholder="Chọn khách hàng tiềm năng"
                onClick={() => {
                    dics[1].toggle();
                }} />
            <Modal
                size="100%"
                title="Danh sách khách hàng tiềm năng" opened={dics[0]} onClose={dics[1].close}
            >
                <MyDataTable
                    enableRowSelection
