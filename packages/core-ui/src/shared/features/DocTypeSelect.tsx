...
export function DocTypeSelect({ documentTypeId, label = "", dataMapper, ...rest }: ISelect) {

    const query = useQuery({
        queryKey: [`/DocumentAttribute/GetByType?documentType=${documentTypeId}`],
        queryFn: async () => {
            const result = (await axiosInstance.get(`/DocumentAttribute/GetByType?documentType=${documentTypeId}`)).data.data;
            return result
        },
    });
