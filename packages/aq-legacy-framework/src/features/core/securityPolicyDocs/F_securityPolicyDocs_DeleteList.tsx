...
export default function F_securityPolicyDocs_DeleteList({ data }: { data: Document[] }) {
  return (
    <CustomButtonDeleteList
      contextData={data.map((data: Document) => `${data.decisionCode}`).join(", ")}
      onSubmit={() => { return documentService.deleteList(data.map((item) => ({ id: item.id } as BaseEntity))) }} />
  );
}
