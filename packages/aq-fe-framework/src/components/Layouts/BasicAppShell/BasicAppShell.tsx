...
import { useMyReactQuery, useQ_AQ_GetAQModule } from "@/hooks";
...
import { CustomSwitchTheme } from "@/core/button/CustomSwitchTheme";
import { Feat_Authenticate_Logout } from "@/modules-features";
import SessionUserInfo from "@/modules-features/basicAppShell/SessionUserInfo";
import { UserAvatarMenu } from "@/modules-features/basicAppShell/UserAvatarMenu";
import { BasicAppShellProps, I_BasicAppShell_LinkItem } from "./types";
import {
...
export function BasicAppShell({ children, menu, extraTopRight, title, logoutRedirect, isDev }: BasicAppShellProps) {
    ...
    const renderNavbar = () => (
        <AppShell.Navbar
        >
            <MyAppSpotlight menu={filteredMenu} />
            ...
            <AppShell.Section className="rounded-tr-2xl shadow-[0_-4px_6px_rgba(0,0,0,0.1)]" style={{
                ...
                <SessionUserInfo />
                <Space m={4} />
                <Feat_Authenticate_Logout redirectURL={logoutRedirect} />
                <Box px={'md'}>
                    <Image
                        fit="contain"
                        h="80px"
